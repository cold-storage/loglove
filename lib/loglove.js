...
var fmt = require('util').format,
  levels = {},
  defaultFormatter = function(name, msg, level) {
...
function determineLevel(name, level) {
    return levels[config[name]] || levels[level] || levels.DEFAULT_LEVEL;
}
...
module.exports.getLog = function getLog(name, level, noshow) {
...
levels.INFO = 6;
levels.DEBUG = 7;
levels.DEFAULT_LEVEL = levels.INFO;
...
module.exports.configure = function(config) {
  ...
  for (key in config) {
    logger = logmap[key];
    if (logger) {
      logger.level = levels[config[key]];
    }
  }
};
...
Log.prototype = {
  ...
  __log: function(level, args) {
    if (this.level >= levels[level]) {
      ...
      this.message = formatter(this.name, this.message, level);
      if (!this.noshow) {
