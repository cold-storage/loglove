...
// ```
// If you use this method you probably want to
// set ```process.env.LOG_LOVE_ROOT_PATH```
// to the base path of your app so your logger names
// are relative to that.
//
// You can safely use loglove in library modules provided you
// specify ```OFF``` as the level.
// ```
...
//
// The configuration defaults to ```{ "/*": "OFF" }```.
//
...
//
// The following will make all your logger names relative
// to ```/a/b/c```
// ```
// process.env.LOG_LOVE_ROOT_PATH="/a/b/c"
// ```
...
function configure(config) {
  ...
  if (!ll.isInitialized) {
    ll.isInitialized = true;
    new Log('/LOGLOVE').info('loglove initialized', ll.config);
  }
  __doReloadLogic();
  return this;
}
...
function __doReloadLogic() {
  if (ll.config['RELOAD_INTERVAL_SECONDS'] && ll.LOG_LOVE_CONFIG_FILE) {
    setTimeout(function() {
      fs.readFile(ll.LOG_LOVE_CONFIG_FILE, {
        encoding: 'utf8'
      }, function(err, cfg) {
        if (!err) {
          configure(cfg);
...
Log.prototype = {
  ...
  __log: function(level, args) {
    if (this.level >= ll[level]) {
      try {
        ...
        this.__message = 'formatter error: ' + err;
      }
      if (!this.__noshow) {
  ...
  __configure: function __configure() {
    this.strength = -1;
    if (ll.LOG_LOVE_ROOT_PATH &&
      this.name.indexOf(ll.LOG_LOVE_ROOT_PATH) === 0) {
      this.name = this.name.substr(ll.LOG_LOVE_ROOT_PATH.length);
    }
...
function __reset() {
  delete ll.isInitialized;
  if (process.env.LOG_LOVE_ROOT_PATH) {
    ll.LOG_LOVE_ROOT_PATH = process.env.LOG_LOVE_ROOT_PATH;
  } else {
    ll.LOG_LOVE_ROOT_PATH = null;
  }
  if (process.env.LOG_LOVE_CONFIG_FILE) {
    ll.LOG_LOVE_CONFIG_FILE = process.env.LOG_LOVE_CONFIG_FILE;
  } else {
    ll.LOG_LOVE_CONFIG_FILE = null;
  }
  ll.logmap = {};
  ll.config = {
    '/*': 'OFF'
  ...
  var cfg = {};
  if (ll.LOG_LOVE_CONFIG_FILE) {
    try {
      cfg = fs.readFileSync(ll.LOG_LOVE_CONFIG_FILE, {
        encoding: 'utf8'
      });
    } catch (e) {
  ...
  configure(cfg);
  // Default formatter outputs date, level, message (in color) and logger name.
  // ```
