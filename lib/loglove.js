...
//
// TODO Implement wildcards (properly).
//
...
exports.configure = function(config) {
  ...
  var cfgLgName, cfgLvlName, logger, logname;
  for (cfgLgName in config) {
    cfgLvlName = config[cfgLgName];
    ll.config[cfgLgName] = cfgLvlName;
    if (cfgLgName.indexOf('/**', cfgLgName.length - 3) !== -1) {
      for (logname in ll.logmap) {
        if (logname.indexOf(cfgLgName.slice(0, -2)) === 0) {
          ll.logmap[logname].level = ll[cfgLvlName];
          ll.logmap[logname].levelName = cfgLvlName;
        }
      }
    } else {
      logger = ll.logmap[cfgLgName];
      if (logger) {
        logger.level = ll[cfgLvlName];
        logger.levelName = cfgLvlName;
      }
    }
    if ('/' === cfgLgName) {
      ll.DEFAULT_LEVEL = ll[cfgLvlName];
    }
  }
...
// ```
// This gives you complete control of how your log messages will look.
...
Log.prototype = {
  ...
  // seems to the case with JavaScript.
